...
<div class="potluckBox" v-for="potluck in potlucks" v-bind:key="potluck.potluckId" :class="{completed: potluck.completed}">
  <div>
    ...
    <i v-if="isHost(potluck.userId)" class="fa-solid fa-star" style="color: #1900ff;"></i><router-link v-bind:to="{name: 'PotluckView', params: {potluckId: potluck.potluckId}}" >{{potluck.eventName}}</router-link>
